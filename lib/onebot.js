...
function imageResource(pic) {
  ...
  const fileNameMd5 = /^[a-f\d]{32}$/i.test(String(pic?.fileName || "")) ? pic.fileName : "";
  const md5 = String(pic?.md5HexStr || pic?.originImageMd5 || fileNameMd5)
    .replace(/[^a-f\d]/gi, "");
  if (md5) return `https://gchat.qpic.cn/gchatpic_new/0/0-0-${md5.toUpperCase()}/0`;

  for (const candidate of [pic?.sourcePath, pic?.filePath, ...thumbnailPaths(pic?.thumbPath)]) {
